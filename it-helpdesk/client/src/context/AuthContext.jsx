...
export const AuthProvider = ({ children }) => {
  const [user, setUser] = useState(() => {
    const stored = localStorage.getItem('deskline_user');
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(({ data }) => {
        setUser(data.user);
        localStorage.setItem('deskline_user', JSON.stringify(data.user));
      })
      .catch(() => {
  ...
  const login = async (email, password) => {
    ...
    localStorage.setItem('deskline_token', data.token);
    localStorage.setItem('deskline_user', JSON.stringify(data.user));
    setUser(data.user);
    return data.user;
  };
